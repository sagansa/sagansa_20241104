// ====================================================================
// Notifikasi procurement — penanganan FCM tipe 'invoice_transfer_created'
// dan 'payment_receipt_paid'.
//
// Backend mengirim push data-only ke app mobile saat:
//   - invoice Transfer dibuat (ke admin/super_admin), atau
//   - payment receipt dibuat (ke created_by tiap record ter-attach).
//
// File ini:
...
//   - Top-level helper [handleProcurementNotificationFcm]: dipanggil oleh
...
/// Tipe payload FCM untuk notifikasi procurement (konvensi sama dengan backend
...
/// ID notifikasi stabil berbasis invoice_id / receipt_id supaya notif baru
/// menimpa (tidak menumpuk) untuk record yang sama.
int _procurementNotificationId(int recordId) => 7000 + (recordId % 1000);

/// Ekstrak id record terkait (invoice_id / receipt_id) dari payload notifikasi.
int? extractRecordId(Map<String, dynamic> payload) {
  final type = payload['type'];
  final raw = type == kFcmTypeInvoiceTransferCreated
      ? payload['invoice_id']
      : payload['receipt_id'];
  return int.tryParse(raw?.toString() ?? '');
}

/// Top-level: handle FCM notifikasi procurement. Dipanggil dari background
/// handler maupun foreground listener. Hanya menampilkan notifikasi lokal —
/// TIDAK boleh menangkap state widget tree (background isolate).
@pragma('vm:entry-point')
Future<void> handleProcurementNotificationFcm(Map<String, dynamic> data) async {
  ...
  // Simpan SELURUH payload (type, id, payment_for) sebagai JSON agar saat
